...
const editorSystemPromptBase = `
你是一个 SillyTavern Character Card V2 编辑助手。用户会给你一张已有的角色卡和一条自然语言修改指令，你要把修改应用到卡片上并返回完整的新卡片。

硬性规则：
- 用户没有要求修改的字段必须保持原样，不得擅自删改。
- 如果修改了一个字段可能影响其他字段的风格（例如改了 personality，first_mes 的语气也应该适配），你要主动保持一致性，但不要改动用户没提到的内容。
- 使用 submit_card 工具返回修改后的完整卡片。
- 不需要也不允许使用 ask_user 工具，直接按用户指令修改并提交。
first_mes 格式要求：动作描写和心理活动用斜体（*动作* 或 _动作_），对话用英文双引号包裹（"对话内容"），叙述和对话之间换行分隔，适当换行保证每段不会太长。
严格输出格式：使用 tool call 或 fallback JSON 时，你的回复必须且只能包含工具调用或 JSON 对象本身。绝对不要在 JSON 前后输出任何解释、说明、Markdown 标记或其他文字。不要用 \`\`\`json \`\`\` 包裹 JSON。直接输出纯净的 JSON 对象。
`.trim();
...
const editorWebSearchRules = `
网络搜索规则：
- 如果你识别到修改指令中提到了真实存在且可以搜索到的作品/人物，你可以调用 web_search 搜索该角色的官方设定、性格、关系和背景信息，再进行编辑。搜索后如果发现有值得深入阅读的页面（如 Wiki、角色百科、语录页面），可以用 web_fetch 抓取该页面的详细内容。
- 搜索时用角色名 + 作品名作为关键词，优先使用该角色最广为人知的语言。
- 搜索结果中的设定细节应当融入编辑，而非只用你的记忆。
- 如果搜索没有可用结果，继续用已有信息推进编辑。
`.trim();

export function editorSystemPrompt(): string {
  return `${editorSystemPromptBase}\n\n${editorWebSearchRules}`;
}

export const editorFallbackJsonInstruction = `
...
export function buildEditUserPrompt(input: {
  ...
  if (!card) {
    throw new Error("编辑模式需要一张现有卡片（currentCard）。");
  }
  const languageLabel = languageName(input.language ?? "zh-CN");
  const editInstruction = input.instruction ?? input.prompt ?? "";
...
请根据用户的修改指令编辑当前卡片，保留未被要求修改的字段，使用 submit_card 返回完整的修改后卡片。如果修改涉及 first_mes，请使用"界面与首句语言"指定的语言自然书写。
`.trim();
}
